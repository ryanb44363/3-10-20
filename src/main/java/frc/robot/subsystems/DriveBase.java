...
public class DriveBase {
    ...
    //private DigitalInput limitSwitchlower = new DigitalInput(Constants.zeroer);
    //private DigitalInput limitSwitchhigher = new DigitalInput(Constants.zeroer);
    ...
    public void arm (XboxController controller1, GenericHID.Hand left, GenericHID.Hand right) {
        ...
        if (controller1.getBButtonPressed()){
            //timer.start();
                //while (timer.get() < 1) {
                  ...
                  //  }
               // while (timer.get() < 1.75 || timer.get() > 1) {
                 //   Arm.set(-.05);
                  //  }
            }
        ...
        if(!limitSwitchhigher.get()) {
            System.out.println("HIGHER Limit Switch Activated!");
            Arm.set(0.0);
          }
